...
export const requestPayment = async params => {
  const response = await PetFriendsPostService('/payments', params);
...
export const getAllPaymentHistory = async userId => {
  const response = await PetFrinedsGetService('/payments/users/' + userId);
  if (response && response.status === 200) {
    console.log({ status: response.status, data: response.data });
    return { status: response.status, data: response.data };
  } else {
...
export const refundPayment = async paymentId => {
  const response = await PetFrinedsPutService('/payments/' + paymentId);
  if (response && response.status === 200) {
    console.log({ status: response.status, data: response.data });
    return { status: response.status, data: response.data };
  } else {
